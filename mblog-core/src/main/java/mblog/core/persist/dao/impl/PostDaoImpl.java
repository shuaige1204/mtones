...
public class PostDaoImpl extends BaseRepositoryImpl<PostPO> implements PostDao {
    ...
    public List<Post> search(Paging paging, String q) throws Exception {
        ...
        SimpleHTMLFormatter formatter = new SimpleHTMLFormatter("<span style='color:red;'>",
            "</span>");
        QueryScorer queryScorer = new QueryScorer(luceneQuery);
        Highlighter highlighter = new Highlighter(formatter, queryScorer);

        List<PostPO> list = query.list();
        List<Post> rets = new ArrayList<Post>(list.size());
        ...
        for (PostPO po : list) {
            ...
            String summary = highlighter.getBestFragment(standardAnalyzer, "summary",
                m.getSummary());
    ...
    public List<PostPO> searchByTag(Paging paigng, String tag) {
        ...
        SearchFactory sf = fullTextSession.getSearchFactory();
        QueryBuilder qb = sf.buildQueryBuilder().forEntity(PostPO.class).get();
        org.apache.lucene.search.Query luceneQuery = qb.phrase().onField("tags").sentence(tag)
            .createQuery();

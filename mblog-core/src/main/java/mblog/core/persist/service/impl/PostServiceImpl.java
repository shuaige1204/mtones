...
import mblog.base.modules.lang.EntityStatus;
import mblog.base.modules.utils.PreviewTextUtils;
...
public class PostServiceImpl implements PostService {
    ...
    public Map<Long, Post> findSingleMapByIds(Set<Long> ids) {
        ...
        for (Entry<Long, Post> entry : rets.entrySet()) {
            if (entry.getValue().getLastImageId() > 0) {
                Attach a = ats.get(entry.getValue().getLastImageId());
                entry.getValue().setAlbum(a);
            }
        }
    ...
    public long post(Post post) {
        ...
        // 更新文章统计
        userEventService
            .identityPost(Collections.singletonList(po.getAuthorId()), po.getId(), true);

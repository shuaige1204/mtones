...
import mblog.base.modules.utils.GMagickUtils;
...
public class ImageUtils extends GMagickUtils {
     ...
     * @throws IOException
     */
    public static boolean scaleImageByWidth(String ori, String dest, int maxSize)
                                                                                 throws IOException {
        File oriFile = new File(ori);
    ...
    public static boolean truncateImage(String ori, String dest, int x, int y, int width, int height) {
        ...
        try {
            validate(oriFile, dest);
            Thumbnails.of(ori).sourceRegion(x, y, width, height).size(width, height)
                .keepAspectRatio(false).toFile(dest);
        } catch (IOException e) {
        }

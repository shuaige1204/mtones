...
import mblog.base.modules.security.MD5;
...
public class BaseController {
    ...
    protected List<Attach> handleAlbums(String[] albums) {
        ...
        for (String album : albums) {
            ...
            try {
                // 保存原图
                String orig = fileRepoFactory.select().storeScale(temp, appContext.getOrigDir(),
                    750);
                item.setOriginal(orig);

                // 创建缩放图片
                String preview = fileRepoFactory.select().storeScale(temp,
                    appContext.getThumbsDir(), 305);
                item.setPreview(preview);

                // 创建快照
                String screenshot = fileRepoFactory.select().storeScale(temp,
                    appContext.getScreenshotDir(), 225, 140);

...
public class CallbackController extends BaseController {
    ...
    public void callWeibo(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            APIConfig.getInstance().setOpenid_sina(
                appContext.getConfig().get(SiteConfig.WEIBO_CLIENT_ID));
            APIConfig.getInstance().setOpenkey_sina(
                appContext.getConfig().get(SiteConfig.WEIBO_CLIENT_SERCRET));
            APIConfig.getInstance().setRedirect_sina(
                appContext.getConfig().get(SiteConfig.SITE_OAUTH_WEIBO));
    ...
    public String callback4Weibo(String code, String state, HttpServletRequest request,
                                 ModelMap model) throws Exception {
        // --
        String session_state = (String) request.getSession().getAttribute(SESSION_STATE);
        // 取消了授权
        if (StringUtils.isBlank(state) || StringUtils.isBlank(session_state)
            || !state.equals(session_state) || StringUtils.isBlank(code)) {
    ...
    public void callQQ(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            APIConfig.getInstance()
                .setOpenkey_qq(appContext.getConfig().get(SiteConfig.QQ_APP_KEY));
            APIConfig.getInstance().setRedirect_qq(
                appContext.getConfig().get(SiteConfig.SITE_OAUTH_QQ));
    ...
    public void callDouban(HttpServletRequest request, HttpServletResponse response) {
        response.setContentType("text/html;charset=utf-8");
        try {
            APIConfig.getInstance().setOpenid_douban(
                appContext.getConfig().get(SiteConfig.DOUBAN_API_KEY));
            APIConfig.getInstance().setOpenkey_douban(
                appContext.getConfig().get(SiteConfig.DOUBAN_SECRET_KEY));
            APIConfig.getInstance().setRedirect_douban(
                appContext.getConfig().get(SiteConfig.SITE_OAUTH_DOUBAN));
    ...
    public String callBack4Douban(String code, String state, HttpServletRequest request,
                                  ModelMap model) {
        // --
        String session_state = (String) request.getSession().getAttribute(SESSION_STATE);
        // 取消了授权
    ...
    public String bindOauth(OpenOauth openOauth, HttpServletRequest request) throws Exception {
        ...
        if (thirdToken != null) {
            ...
            if (user == null) {
                ...
                ImageUtils.download(openOauth.getAvatar(), fileRepoFactory.select().getRoot()
                                                           + ava100);
